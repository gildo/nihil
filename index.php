...
<?php
$ris = $mysql->query($query) or die ("Errore nell'esecuzione della query");
while ($res = mysql_fetch_array ($ris , MYSQL_ASSOC))

{
    	if (is_logged () && login (mysql_real_escape_string ($_COOKIE ['_user']),mysql_real_escape_string ($_COOKIE ['_pass'])))
    	
    	        print("<div id='{$res ['id']}' class='post'>
            <div class='header'>
                <span class='title'> <tr>\n<td> <a href = 'post-{$res ['id']}'>#{$res ['name']}</a> </tr> posted by '{$res ['author']}' at '{$res ['hour']}' of {$res ['date']} <td><a href = 'admin.php?mode=delete_post&id={$res ['id']}'>rm</a>		    <td><a href = 'admin.php?mode=edit_post&id={$res ['id']}'>edit</a></td> </span> <div style='float: right;'></div>
            </div>
        ...
        ");
         ...
         print("<div id='{$res ['id']}' class='post'>
            <div class='header'>
                <span class='title'> <tr>\n<td> <a href = 'post-{$res ['id']}'>#{$res ['name']}</a> </tr> posted by '{$res ['author']}' at '{$res ['hour']}' of {$res ['date']} </span> <div style='float: right;'></div>
            </div>
