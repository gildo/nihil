        ...
        switch ($mode)
        {
            ...
            break;    
           	...
           	case "logout":
        		if (admin_exists ())
        		{
        		    ...
        		    if (is_logged () && login ($mysql->prepare ($_COOKIE ['_user']), $mysql->prepare ($_COOKIE ['_pass'])))
        			{
            ...
            case "show":
                if (isset ($_GET ['id']))
                {
                    ...
                    $ris = $mysql->fetch_array ($mysql->query($query) , MYSQL_ASSOC);
            		print ( $ris ['post'] . "<br>");
...
<?php
    ...
    while ($res = $mysql->fetch_array ($ris , MYSQL_ASSOC))
    {
    	if (is_logged () && login ( $mysql->prepare($_COOKIE ['_user'] ), $mysql->prepare ($_COOKIE ['_pass'])))
