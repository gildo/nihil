...
function emailUsernameLinked($email,$username)
{
	...
	if(returns_result("SELECT Username, Email FROM ".$db_table_prefix."Users WHERE Username_Clean = '".$db->sql_escape(sanitize($username))."' AND Email = '".$db->sql_escape(sanitize($email))."' AND Active = 1") > 0)
		return true;
	else
		return false;
}
